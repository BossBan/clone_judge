...
它可以读取包含代码块索引信息的大型 CSV 文件，从本地指定的目录库（`bcb_reduced`）中提取对应的代码片段，以高并发的方式向模型发送检测请求，并将返回判断结果（`TRUE`, `FALSE` 或 `ERROR`）在终端输出。
...
| `--api-key`           | `str`     | `env: ZAI_API_KEY` | 智谱 AI (或其它支持的) API 密钥。                           |
...
处理过程中，终端会打印出实时进度及预计完成情况。单个 CSV 文件执行完毕后，控制台会输出 `TRUE` 及 `FALSE` 的数量和比例分析。
